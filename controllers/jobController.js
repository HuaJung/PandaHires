...
const getOpenJobs = async(req, res) => {
  ...
  const jobs = await Job.findAll({
    ...
    where: {companyId: foundCompany.id, status: 'Open'}
  })
...
const getOverviewJobs = async (req, res) => {
  ...
  try {
    const applicantInterviewOfferCount = await sequelize.query(
      ...
      COUNT(CASE WHEN DATE(jc.createdAt) = CURDATE() THEN jc.id END) AS applicantToday, 
      ...
      COUNT(CASE WHEN sr.stageId = 26 AND sr.createdAt >= CURDATE() - INTERVAL 7 DAY THEN sr.jobCandidateId END) AS offerPast7Day
      FROM jobCandidates jc
      INNER JOIN jobs ON jc.jobId = jobs.id 
      INNER JOIN stageRecords sr ON jc.id = sr.jobCandidateId
      INNER JOIN companies ON companies.id = jobs.companyId
      WHERE jobs.companyId = 1
